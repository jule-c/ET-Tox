...
class DataModule(LightningDataModule):
    def __init__(self, hparams, dataset=None, multi_conformer_training=False):
        super(DataModule, self).__init__()
        self.save_hyperparameters(hparams)
        self._mean, self._std = None, None
        self._saved_dataloaders = dict()
        self.dataset = dataset
        self.multi_conformer_training = multi_conformer_training
    ...
    def setup(self, stage):
        if self.dataset is None:
            if self.hparams["dataset"] == "Custom":
                ...
                if self.multi_conformer_training:
                    self.dataset_val_test = getattr(datasets, self.hparams["dataset"]+"Single")(
                        self.hparams["dataset_root"], **self.hparams["dataset_arg"]
                    )
            ...
            if self.multi_conformer_training:
                ...
                self.val_dataset = Subset(self.dataset_val_test, self.idx_val)
                self.test_dataset = Subset(self.dataset_val_test, self.idx_test)

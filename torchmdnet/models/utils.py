...
class NeighborEmbedding(MessagePassing):
    def __init__(self, hidden_channels, num_rbf, cutoff_lower, cutoff_upper, max_z=10, num_atoms=10, load_from_diffusion=False):
        super(NeighborEmbedding, self).__init__(aggr="add")

        if load_from_diffusion:
...
class GatedEquivariantLinearLayer(nn.Module):
    """Gated Equivariant Layer as defined in Schütt et al. (2021):
    Equivariant message passing for the prediction of tensorial properties and molecular spectra
    """
    ...
    def __init__(
        ...
        super(GatedEquivariantLinearLayer, self).__init__()
        self.out_channels = out_channels

        self.vec1_proj = nn.Linear(hidden_channels, hidden_channels, bias=False)

        self.update_net = nn.Linear(hidden_channels*2, out_channels)

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.vec1_proj.weight)
        nn.init.xavier_uniform_(self.update_net.weight)
        self.update_net.bias.data.fill_(0)

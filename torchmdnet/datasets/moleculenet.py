...
class MoleculeNet(Dataset):
    ...
    def get(self, idx):
        ...
        import random
        with open(f'{self.dataset}_multi_conf.pickle', 'rb') as f:
           data_list = pickle.load(f)[idx]
        data = data_list[0]
        slices = data_list[1]
        num_confs = len(data.y)
        rd_int = random.choice([i for i in range(num_confs)])
        data = self.getter(data, slices, rd_int)
        print(f"Randomly selected conformer {rd_int} out of {num_confs} conformers\n")

...
def parse_xtb_xyz(filename, select_random=False):
...
class TDCToxSingle(Dataset):
    ...
    def __init__(
        ...
        self.Q_mm = np.memmap(y_name, mode="r", dtype=np.float32)
    ...
    def sample_iter(self):
        ...
        for i, d in tqdm(csv_file.iterrows()):
            if i in mol_indices:
                ...
                mol_dict = parse_xtb_xyz(file, self.conformer=='random')
                j += 1
            ...
            elif i not in mol_indices and not self.dataset == 'mutagenicity':
                labels = {
                    'tox_labels': torch.tensor(tox_label, dtype=torch.float32),
                    'Q': torch.tensor(Q, dtype=torch.float32),
                    'smiles': smiles,
